...
//    else left.filterAcc(p, right.filterAcc(p, acc))
  ...
  def mostRetweetedAcc(most: Tweet): Tweet = {
    val max: Tweet = if (elem.retweets > most.retweets) elem else most
  ...
  /**
    ...
    * and be implemented in the subclasses?
    */
  def descendingByRetweet: _root_.objsets.TweetList = {
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter {
      tweet => {
        google exists { googleString => tweet.text.contains(googleString) }
      }
    }
  ...
  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter {
      tweet => {
        apple exists { appleString => tweet.text.contains(appleString) }
      }
    }
